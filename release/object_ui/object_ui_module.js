...
contextMenu.appendItem(Common.UIString('Show as JavaScript object'),this._disassemble.bind(this));contextMenu.appendApplicableItems(this._object);contextMenu.show();}
...
contextMenu.appendApplicableItems(property.value);var copyPathHandler=InspectorFrontendHost.copyText.bind(InspectorFrontendHost,this.nameElement.title);contextMenu.beforeShow(()=>{contextMenu.appendItem(Common.UIString('Copy property path'),copyPathHandler);});contextMenu.show();}
...
return[];var selectedFrame=executionContext.debuggerModel.selectedCallFrame();if(!expressionString&&selectedFrame){return completionsOnPause(selectedFrame);}else{var result=await executionContext.evaluate({expression:expressionString,objectGroup:'completion',includeCommandLineAPI:true,silent:true,returnByValue:false,generatePreview:false},false,false);return completionsOnGlobal(result);}
...
executionContext.runtimeModel.releaseObjectGroup('completion');return receivedPropertyNames(completions);function getCompletions(type){var object;if(type==='string')
