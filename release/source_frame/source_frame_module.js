...
return;this._initialized=true;var obj=SDK.RemoteObject.fromLocalObject(this._parsedJSON.data);var title=this._parsedJSON.prefix+obj.description+this._parsedJSON.suffix;this._treeOutline=new ObjectUI.ObjectPropertiesSection(obj,title);this._treeOutline.enableContextMenu();this._treeOutline.setEditable(false);this._treeOutline.expand();this.element.appendChild(this._treeOutline.element);}
...
return new UI.EmptyWidget(Common.UIString('Nothing to preview'));var resourceType=Common.ResourceType.fromMimeType(mimeType);if(resourceType===Common.resourceTypes.Other)
...
var parsedXML=SourceFrame.XMLView.parseXML(content,mimeType);if(parsedXML)
...
return SourceFrame.JSONView.createSearchableView((parsedJSON));if(resourceType.isTextType()){var highlighterType=mimeType.replace(/;.*/,'');return SourceFrame.ResourceSourceFrame.createSearchableView(provider,highlighterType);}
