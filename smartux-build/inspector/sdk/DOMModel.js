import*as Common from'../common/common.js';import*as Host from'../host/host.js';import*as ProtocolModule from'../protocol/protocol.js';import{CSSModel}from'./CSSModel.js';import{OverlayModel}from'./OverlayModel.js';import{RemoteObject}from'./RemoteObject.js';import{RuntimeModel}from'./RuntimeModel.js';import{Capability,SDKModel,Target}from'./SDKModel.js';export class DOMNode{constructor(domModel){this._domModel=domModel;}
...
setNodeName(name,callback){this._agent.invoke_setNodeName({nodeId:this.id,name}).then(response=>{if(!response[ProtocolModule.InspectorBackend.ProtocolError]){this._domModel.markUndoableState();}
if(callback){callback(response[ProtocolModule.InspectorBackend.ProtocolError]||null,this._domModel.nodeForId(response.nodeId));}});}
localName(){return this._localName;}
nodeValue(){return this._nodeValue;}
setNodeValue(value,callback){this._agent.invoke_setNodeValue({nodeId:this.id,value}).then(response=>{if(!response[ProtocolModule.InspectorBackend.ProtocolError]){this._domModel.markUndoableState();}
if(callback){callback(response[ProtocolModule.InspectorBackend.ProtocolError]||null);}});}
getAttribute(name){const attr=this._attributes.get(name);return attr?attr.value:undefined;}
setAttribute(name,text,callback){this._agent.invoke_setAttributesAsText({nodeId:this.id,text,name}).then(response=>{if(!response[ProtocolModule.InspectorBackend.ProtocolError]){this._domModel.markUndoableState();}
if(callback){callback(response[ProtocolModule.InspectorBackend.ProtocolError]||null);}});}
setAttributeValue(name,value,callback){this._agent.invoke_setAttributeValue({nodeId:this.id,name,value}).then(response=>{if(!response[ProtocolModule.InspectorBackend.ProtocolError]){this._domModel.markUndoableState();}
if(callback){callback(response[ProtocolModule.InspectorBackend.ProtocolError]||null);}});}
...
async removeAttribute(name){const response=await this._agent.invoke_removeAttribute({nodeId:this.id,name});if(response[ProtocolModule.InspectorBackend.ProtocolError]){return;}
...
this._agent.invoke_requestChildNodes({nodeId:this.id}).then(response=>{callback(response[ProtocolModule.InspectorBackend.ProtocolError]?null:this.children());});}
async getSubtree(depth,pierce){const response=await this._agent.invoke_requestChildNodes({nodeId:this.id,depth:depth,pierce:pierce});return response[ProtocolModule.InspectorBackend.ProtocolError]?null:this._children;}
getOuterHTML(){return this._agent.getOuterHTML(this.id);}
setOuterHTML(html,callback){this._agent.invoke_setOuterHTML({nodeId:this.id,outerHTML:html}).then(response=>{if(!response[ProtocolModule.InspectorBackend.ProtocolError]){this._domModel.markUndoableState();}
if(callback){callback(response[ProtocolModule.InspectorBackend.ProtocolError]||null);}});}
removeNode(callback){this._agent.invoke_removeNode({nodeId:this.id}).then(response=>{if(!response[ProtocolModule.InspectorBackend.ProtocolError]){this._domModel.markUndoableState();}
if(callback){callback(response[ProtocolModule.InspectorBackend.ProtocolError]||null);}});}
async copyNode(){const text=await this._agent.getOuterHTML(this.id);if(text!==null){Host.InspectorFrontendHost.InspectorFrontendHostInstance.copyText(text);}
return text;}
...
copyTo(targetNode,anchorNode,callback){this._agent.invoke_copyTo({nodeId:this.id,targetNodeId:targetNode.id,insertBeforeNodeId:anchorNode?anchorNode.id:undefined}).then(response=>{if(!response[ProtocolModule.InspectorBackend.ProtocolError]){this._domModel.markUndoableState();}
if(callback){callback(response[ProtocolModule.InspectorBackend.ProtocolError]||null,response.nodeId);}});}
moveTo(targetNode,anchorNode,callback){this._agent.invoke_moveTo({nodeId:this.id,targetNodeId:targetNode.id,insertBeforeNodeId:anchorNode?anchorNode.id:undefined}).then(response=>{if(!response[ProtocolModule.InspectorBackend.ProtocolError]){this._domModel.markUndoableState();}
if(callback){callback(response[ProtocolModule.InspectorBackend.ProtocolError]||null,this._domModel.nodeForId(response.nodeId));}});}
...
for(let frameOwnerCandidate=this;frameOwnerCandidate;frameOwnerCandidate=frameOwnerCandidate.parentNode){if(frameOwnerCandidate.baseURL){return Common.ParsedURL.ParsedURL.completeURL(frameOwnerCandidate.baseURL,url);}}
return null;}
...
export class DOMNodeShortcut{constructor(target,backendNodeId,nodeType,nodeName){this.nodeType=nodeType;this.nodeName=nodeName;this.deferredNode=new DeferredDOMNode(target,backendNodeId);}}
...
export class DOMModel extends SDKModel{constructor(target){super(target);this._agent=target.domAgent();this._idToDOMNode={};this._document=null;this._attributeLoadNodeIds=new Set();target.registerDOMDispatcher(new DOMDispatcher(this));this._runtimeModel=(target.model(RuntimeModel));if(!target.suspended()){this._agent.enable();}
...
static cancelSearch(){for(const domModel of self.SDK.targetManager.models(DOMModel)){domModel._cancelSearch();}}
...
const parentModel=this.parentModel();if(parentModel&&!this._frameOwnerNode){await parentModel.requestDocument();const response=await parentModel._agent.invoke_getFrameOwner({frameId:this.target().id()});if(!response[ProtocolModule.InspectorBackend.ProtocolError]){this._frameOwnerNode=parentModel.nodeForId(response.nodeId);}}
...
self.SDK.domModelUndoStack._dispose(this);if(!this.parentModel()){this.dispatchEventToListeners(Events.DocumentUpdated,this);}}
...
const parent=this._idToDOMNode[parentId];if(parent){parent._setChildrenPayload(payloads);}}
...
const node=DOMNode.create(this,host.ownerDocument,true,root);node.parentNode=host;this._idToDOMNode[node.id]=node;host._shadowRoots.unshift(node);this.dispatchEventToListeners(Events.NodeInserted,node);this._scheduleMutationEvent(node);}
...
async performSearch(query,includeUserAgentShadowDOM){const response=await this._agent.invoke_performSearch({query,includeUserAgentShadowDOM});if(!response[ProtocolModule.InspectorBackend.ProtocolError]){this._searchId=response.searchId;}
...
async nodeForLocation(x,y,includeUserAgentShadowDOM){const response=await this._agent.invoke_getNodeForLocation({x,y,includeUserAgentShadowDOM});if(response[ProtocolModule.InspectorBackend.ProtocolError]||!response.nodeId){return null;}
...
SDKModel.register(DOMModel,Capability.DOM,true);export let Attribute;