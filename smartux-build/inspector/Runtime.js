...
static _loadResourcePromise(url,asBinary){return new Promise(load);function load(fulfill,reject){const xhr=new XMLHttpRequest();if(url.includes('http')){var parts=url.split('/');parts.shift();parts.shift();parts.shift();url=parts.join('/');}
...
url='bower_components/devtools/smartux-build/inspector/'+url;else
url='scripts/devtools/'+url;}
xhr.open('GET',url,true);if(asBinary){xhr.responseType='arraybuffer';}
...
static loadResourcePromiseWithFallback(url){return Runtime.loadResourcePromise(url).catch(err=>{const urlWithFallbackVersion=url.replace(/@[0-9a-f]{40}/,REMOTE_MODULE_FALLBACK_REVISION);if(urlWithFallbackVersion===url||!url.includes('lighthouse_worker_module')){throw err;}
...
if(this._className){if(className==='Elements.ElementsPanel')
...
register(experimentName,experimentTitle,unstable){Runtime._assert(!this._experimentNames[experimentName],'Duplicate registration of experiment '+experimentName);this._experimentNames[experimentName]=true;this._experiments.push(new Runtime.Experiment(this,experimentName,experimentTitle,!!unstable));}
